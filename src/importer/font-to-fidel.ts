...
function flattenPath(path: opentype.Path, steps = 5): Segment[] {
      ...
      for (let col = 0; col < width; col++) {
        ...
        for (let dr = 0; dr < 4; dr++) {
          ...
          for (let dc = 0; dc < 2; dc++) {
            const testX = x1 * scale + ((col * 2 + dc + 0.5) / subCols) * glyphWidth;
            const testY = -(y2 * scale - ((row * 4 + dr + 0.5) / subRows) * glyphHeight);
...
export async function importFont(path: string, options: ImportOptions): Promise<FidelFont> {
  const font = await opentype.load(path);
  const glyphs: { [key: string]: string[] } = {};
  const style = options.style || 'blocks';
  
  const ranges = [[0x1200, 0x137F], [0x1380, 0x139F], [0x2D80, 0x2DDF], [0xAB00, 0xAB2F]];

  for (const [start, end] of ranges) {
    for (let i = start; i <= end; i++) {
      const char = String.fromCharCode(i);
      const glyph = font.charToGlyph(char);
      if (glyph && (glyph as any).unicode !== undefined) {
  ...
  return {
    metadata: {
      name: `${font.names.fontFamily.en || "Imported Font"} (${style})`,
